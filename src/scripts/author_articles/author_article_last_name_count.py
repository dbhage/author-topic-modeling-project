...
from scripts import GALENET_AUTHOR_MASTER_LIST_FNAME, AUTHOR_ARTICLE_LNC_CSV_FNAME, CITATIONS_FILE_FNAME
import time, os
...
with open(AUTHOR_ARTICLE_LNC_CSV_FNAME, 'w') as fd:
    ...
    for article in file_names:
        ...
        fd.write(article + ',' + str(max_auth_tup[0]) + ',' + str(max_auth_tup[1]) + ',' + citations_dict[article].title + '\n')
